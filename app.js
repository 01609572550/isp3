(function () {
  ...
  function stats() {
    ...
    const monthTransactions = state.transactions.filter((t) => t.date.slice(0, 7) === currentMonth);
    return {
      ...
      todayCollection: state.transactions.filter((t) => t.date.slice(0, 10) === today).reduce((sum, t) => sum + t.amount, 0),
